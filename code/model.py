...
print(train_data)
print(dev_data)
train_data=train_data.shuffle(100).batch(32, drop_remainder=True)
dev_data= dev_data.shuffle(100).batch(32, drop_remainder=True)
...
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-5, decay=1e-6), loss=tf.keras.losses.CategoricalCrossentropy(),
